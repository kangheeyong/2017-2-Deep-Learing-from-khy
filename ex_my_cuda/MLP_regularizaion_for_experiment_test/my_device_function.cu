...
__global__ void elu(float *a,float *z,long n)
{
    long tid = blockIdx.x*blockDim.x + threadIdx.x;
    while(tid < n)
    {
        if(z[tid] > 0 ) a[tid] = z[tid];
        else a[tid] = expf(z[tid]) - 1;      

        tid+= blockDim.x * gridDim.x;
    }
}

__global__ void elu_inv(float *a,float *z,long n)
{
    long tid = blockIdx.x*blockDim.x + threadIdx.x;
    while(tid < n)
    { 
        if(z[tid] > 0 ) a[tid] = 1.0;
        else a[tid] = expf(z[tid]);      
        tid+= blockDim.x * gridDim.x;
    }
}
...
__global__ void new_activation(float *a,float *z,long n)
{
 long tid = blockIdx.x*blockDim.x + threadIdx.x;
    while(tid < n)
    { 
        if(z[tid] > 0 ) a[tid] = 2*logf(z[tid] + 1);
        else a[tid] = -2*logf(-z[tid] + 1);      
        tid+= blockDim.x * gridDim.x;
    }
...
__global__ void new_activation_inv(float *a,float *z,long n)
{
 long tid = blockIdx.x*blockDim.x + threadIdx.x;
    while(tid < n)
    { 
        if(z[tid] > 0 ) a[tid] = 2/(1+z[tid]);
        else a[tid] = 2/(-z[tid]+1);      
        tid+= blockDim.x * gridDim.x;
    }
...
__global__ void adam_mean(float *adam_mean, float* delta, float beta1,long n)
{
  ...
  while(tid < n)
  {
      adam_mean[tid] = (beta1*adam_mean[tid] + (1.0- beta1)*delta[tid]);
      tid+= blockDim.x * gridDim.x;
  }
}
...
__global__ void adam_var(float *adam_var, float* delta, float beta2,long n)
{
  ...
  while(tid < n)
  {
      adam_var[tid] = (beta2*adam_var[tid] + (1.0- beta2)*delta[tid]*delta[tid]);
      tid+= blockDim.x * gridDim.x;
  }
}
...
__global__ void adam_sum(float *result, float *adam_mean,float *adam_var,float beta1_t,float beta2_t,long n)
{
  ...
  while(tid < n)
  {
      result[tid] = (adam_mean[tid]/(1-beta1_t))/(sqrtf(adam_var[tid]/(1-beta2_t)) + 0.00000001);
      tid+= blockDim.x * gridDim.x;
  }
}
__global__ void maxnorm_constraints(float *a, float max,long n)
{
  ...
  while(tid < n)
  {
      if(a[tid] > max) a[tid] = max;
      else if(a[tid] < -max) a[tid] = -max;
      tid+= blockDim.x * gridDim.x;
  }
}
...
__global__ void inverted_dropout(float *dropout,float *probability, float dropout_rate,long n)
{
  ...
  while(tid < n)
  {
      if(probability[tid] > dropout_rate) dropout[tid] = 0.0;
      else dropout[tid] = 1.0/dropout_rate; 
      tid+= blockDim.x * gridDim.x;
  }
}
