...
unsigned long print_per_iteration = 100000;
...
int main(int argc,char **argv)
{
    ...
    if(argc != 4)
    {
        printf("USAGE : %s  [gpu_id] [result.txt]\n",argv[0]);
        exit(1);
    }
    ...
    if(max_data > 60000) max_data = 60000; 
    ...
    alpha = 0.00001;//(rand()%9 + 1)*pow(10,-(rand()% 5 + 2)); // 0.09 ~ 0.000001
    train_batch_size =  100;//rand()%500 + 50; //50~550
    maxnorm_constraints = 2;//(rand()%40 + 15)/10.0; // 1.5~5.5
    ...
    //make_index(maximum_train_set  - train_batch_size);
    make_index(max_data - train_batch_size);
    for(long iter = 1 ; iter <= max_iteration; iter++){
        
        pthread_mutex_lock(&mutex);

        idx = get_next();// + maximum_train_set;
        sync_stat = true;
        ...
        if(iter% print_per_iteration == 0)
        {
            ...
            // network->test_setting(validation_batch_size);
            // network->test_forward_propagation(validation_batch_size);
    ...
    fprintf(fd,"%1.8f, %3ld, %2.2f, %2.6f, %1.4f,%3d\n",alpha,train_batch_size,maxnorm_constraints,test_error,test_accuracy,max_data);
