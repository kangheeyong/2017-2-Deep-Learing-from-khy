...
__global__ void my_kernel(float *a,float *b, float *c, int m, int k)
{
  ...
  while(tid < k*k)
  {
      ...
      for(int i = 0 ; i < m ;i++)
      {
          c[IDX2C(y,x,k)] += a[IDX2C(i,y,m)]*b[IDX2C(i,x,m)];
      }
...
int main(int argc, char** argv)
{
    ...
    matrix2 = new float[m*k];
    matrix3 = new float[k*k];
    ...
    cudaMalloc(&d_matrix2,m*k*sizeof(float));
    cudaMalloc(&d_matrix3,k*k*sizeof(float));

    // memory -> cuda memory
    t = clock();
    cublasCreate(&handle);
    cublasSetMatrix(m,k,sizeof(float),matrix1,m,d_matrix1,m);
    cublasSetMatrix(m,k,sizeof(float),matrix2,m,d_matrix2,m);
    cublasSetMatrix(k,k,sizeof(float),matrix3,k,d_matrix3,k);
    ...
    //행렬 좌표가 먼저 설정된 다음에 transpose가 
    //
    //stat = cublasSgemm(handle,  CUBLAS_OP_T,CUBLAS_OP_N,k,k,1,  &al,  d_matrix1,1,  d_matrix2,1,  &bet,  d_matrix3,k);
    int threadsPerBolck = 1024;
    int blocksPerGride = (k*k + threadsPerBolck -1)/threadsPerBolck;
    my_kernel<<<blocksPerGride, threadsPerBolck>>>(d_matrix1,d_matrix2,d_matrix3,m,k);
    ...
    cublasGetMatrix(m,k,sizeof(float),d_matrix2,m,matrix2,m);
    cublasGetMatrix(k,k,sizeof(float),d_matrix3,k,matrix3,k);
