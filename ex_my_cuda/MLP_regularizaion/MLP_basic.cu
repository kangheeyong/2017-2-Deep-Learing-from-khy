...
__global__ void dropout(float *a,float *probability, float dropout_rate,long n)
{
  long tid = blockIdx.x*blockDim.x + threadIdx.x;
  while(tid < n)
  {
      if(probability[tid] > dropout_rate) a[tid] = 0.0;
...
void MLP_basic :: train_forward_propagation(long batch_size,float *dropout_rate)
{
    ...
    {
        // a(i) -> drop out -> a(i)
        CURAND_CALL(curandGenerateUniform(rand_gen,d_temp,neural[i+1]*batch_size));
        blocksPerGride = (neural[i+1]*batch_size + threadsPerBolck -1)/threadsPerBolck;
        dropout<<<blocksPerGride, threadsPerBolck>>>(d_a[i],d_temp,dropout_rate[i],neural[i+1]*batch_size);
        ...
        //CURAND_CALL(curandGenerateUniform(rand_gen,d_temp,neural[i+1]*batch_size));
        //blocksPerGride = (neural[i+1]*batch_size + threadsPerBolck -1)/threadsPerBolck;
        //dropout<<<blocksPerGride, threadsPerBolck>>>(d_a[i+1],d_temp,dropout_rate[i],neural[i+1]*batch_size);
...
__global__ void multi_dropout(float *a,float dropout_rate,long n)
{
  long tid = blockIdx.x*blockDim.x + threadIdx.x;
  while(tid < n)
  {
      a[tid] = dropout_rate*a[tid];
      tid+= blockDim.x * gridDim.x;
  }
}
...
void MLP_basic :: test_forward_propagation(long batch_size,float *dropout_rate)
{
    float one = 1.0;
    float zero = 0.0;
    long threadsPerBolck = 1024;
    ...
    for(int i = 0 ; i < total_layers-2; i++)
    {
        ...
        // z(i+1) = z(i+1)*dropout_rate(i) 
        blocksPerGride = (neural[i+1]*batch_size + threadsPerBolck -1)/threadsPerBolck;
        multi_dropout<<<blocksPerGride, threadsPerBolck>>>(d_z[i+1],dropout_rate[i],neural[i+1]*batch_size);
...
void MLP_basic :: temp_print()
{
    ...
    int mini_batch = 10;
    int total = 3;
    stat = cublasGetMatrix(neural[total],mini_batch,sizeof(float),d_a[total],neural[total],aaa,neural[total]);
  
    cout<<stat<<endl;

    for(int y = 0 ; y < neural[total] ; y++)
    {
        for(int x = 0 ; x < mini_batch ;x++)
        {
            printf("%1.4f ",aaa[IDX2C(y,x,neural[total])]);
        }
        cout<<endl;
    }
    cout<<endl; 
