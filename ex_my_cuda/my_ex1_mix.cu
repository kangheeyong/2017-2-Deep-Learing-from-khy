...
__global__ void my_kernel(float *a,int n)
{
  ...
  while(tid < n)
  {
     ...
     if(a[tid] > 0) a[tid] = logf(a[tid] + 1);
     else a[tid] = -logf(-a[tid] + 1);
...
int main(int argc, char** argv)
{
    ...
    unsigned int m = 6000; //row
    unsigned int n = 4000; //column
    unsigned int k = 5000;
...
*/
    ...
    //
    //
    stat = cublasSgemm(handle,  CUBLAS_OP_N,CUBLAS_OP_N,m-4,n-2,k-3,  &al,  d_matrix1+m,m,  d_matrix2,k,  &bet,  d_matrix3,m);
    int threadsPerBolck = 1024;
    int blocksPerGride = (m*n + threadsPerBolck -1)/threadsPerBolck;
    my_kernel<<<blocksPerGride, threadsPerBolck>>>(d_matrix3,m*n);
