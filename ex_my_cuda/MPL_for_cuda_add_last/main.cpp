...
unsigned long print_per_iteration = 1000;
...
int main(int argc,char **argv)
{
    ...
    network.init(neurals,layers,max_batch_size,alpha,ramda);
    ...
    for(long iter = 1 ; iter <= max_iteration; iter++){
        ...
        network.third_train_set_host_device(data.cur_input, data.cur_target,train_batch_size); 
        ...
        network.activation(train_batch_size);
 ...
 //       network.temp_print(); 
        ...
        network.update(train_batch_size);
    ...
    network.activation(test_batch_size);
