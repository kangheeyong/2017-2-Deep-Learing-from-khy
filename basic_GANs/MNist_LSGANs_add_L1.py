...
if not os.path.exists('out_lsgans_L1/'):
    os.makedirs('out_lsgans_L1/')
...
for it in range(max_epoch) :
    ...
    if it % 500 == 0:
        ...
        if i == 0 :
            fig = plot(test)
            plt.savefig('out_lsgans_L1/{}_origin.png'.format(str(i).zfill(3)), bbox_inches='tight')
            plt.close(fig)

        samples = sess.run(G_sample, feed_dict={Z: test})

        fig = plot(samples)
        plt.savefig('out_lsgans_L1/{}_gener.png'.format(str(i).zfill(3)), bbox_inches='tight')
        i += 1
        plt.close(fig)
