...
if not os.path.exists('out_lsgans/'):
    os.makedirs('out_lsgans/')
...
D_loss = 0.5*tf.reduce_mean((D_real-1)**2 + D_fake**2)/(2*mini_batch_size)
G_loss = 0.5*tf.reduce_mean((D_fake-1)**2)/mini_batch_size
...
for it in range(max_epoch) :
    ...
    if it % 500 == 0:
        ...
        if i == 0 :
            fig = plot(test)
            plt.savefig('out_lsgans/{}_origin.png'.format(str(i).zfill(3)), bbox_inches='tight')
            plt.close(fig)

        samples = sess.run(G_sample, feed_dict={Z: test})

        fig = plot(samples)
        plt.savefig('out_lsgans/{}_gener.png'.format(str(i).zfill(3)), bbox_inches='tight')
        i += 1
        plt.close(fig)
