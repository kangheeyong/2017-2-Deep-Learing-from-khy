...
void EXAMPLE_MNIST_CLASS :: second_init(int batch_size)
{
    ...
    {
        test_labels[IDX2C((int)buff[i],i,MAX_LABEL_NUM)] = 1.0;
    }
    ...
    {
        validation_labels[IDX2C((int)buff[i],i,MAX_LABEL_NUM)] = 1.0;
    }
 ...
 /* 
    for(int y = 0 ; y < 28 ; y++)
    {
        for(int x = 0 ; x < 28 ; x++)
        {
            printf("%1.1f ",validation_images[IDX2C(y,x,28)]);
          //  cout<<test_images[784+IDX2C(y,x,28)]<<" ";
        }
        cout<<endl;
    }
*/
...
void EXAMPLE_MNIST_CLASS :: third_read_train(int index)
{
  ...
  {
      cur_target[IDX2C((int)buff[i],i,MAX_LABEL_NUM)] = 1.0;
  }
