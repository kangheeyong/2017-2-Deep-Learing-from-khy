...
W1 = tf.Variable(tf.random_normal(shape = [784, 3074], mean = 0,stddev = 0.1,seed = my_seed))
b1 = tf.Variable(tf.random_normal(shape = [3074], mean = 0,stddev = 0.1,seed = 2*my_seed))
W2 = tf.Variable(tf.random_normal(shape = [3074, 3074], mean = 0,stddev = 0.1,seed = 3*my_seed))
b2 = tf.Variable(tf.random_normal(shape = [3074], mean = 0,stddev = 0.1,seed = 4*my_seed))
W3 = tf.Variable(tf.random_normal(shape = [3074, 10], mean = 0,stddev = 0.1,seed = 5*my_seed))
...
for i in range(max_iteration) :
    ...
    res2 = sess.run([cross_entropy,accuracy],feed_dict = {X : test_input, T : test_label})
    validation_error.append(res2[0])
    validation_accuracy.append(res2[1])
    if i % 100 == 0 :
        print(i, res[1], res[2], res2[0],res2[1])

res2 = sess.run([cross_entropy,accuracy,W1,W2,W3],feed_dict = {X : test_input, T : test_label})
...
np.savetxt(fname='no_BN_' + folder_name + '/W1.txt',X=res2[2],fmt="%f")
np.savetxt(fname='no_BN_' + folder_name + '/W2.txt',X=res2[3],fmt="%f")
np.savetxt(fname='no_BN_' + folder_name + '/W3.txt',X=res2[4],fmt="%f")
